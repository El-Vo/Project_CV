...
class ObjectScanner:
    def __init__(self, device=None, db_folder="faiss_db"):
        ...
        sam2_model = load_model(variant="tiny", ckpt_path=sam2_checkpoint, device=self.device)
    ...
    def process_and_store(self, frame, bbox, label):
        ...
        # Extract Vector
        inputs = self.dino_processor(images=Image.fromarray(cv2.cvtColor(obj_crop, cv2.COLOR_BGR2RGB)), return_tensors="pt").to(self.device)
        with torch.no_grad():
            outputs = self.dino_model(**inputs)
            feat = torch.nn.functional.normalize(outputs.last_hidden_state[:, 0, :], p=2, dim=-1)
    ...
    def run_scanning(self):
        ...
        while cap.isOpened():
            ...
            if not tracker_initialized:
                ...
                cv2.putText(display, "Center object on red dot, press SPACE to start tracking", 
                ...
                if key == 32:  # SPACE
                    ...
                    if len(y_idx) > 0 and len(x_idx) > 0:
                        ...
                        print("Tracker initialized! Press SPACE to capture views, ESC to exit.")
                    else:
            ...
            # STAGE 2: Tracking - update tracker and allow captures
                ...
                if success:
                    ...
                    cv2.putText(display, f"Tracking {name} - SPACE to capture, ESC to finish", 
